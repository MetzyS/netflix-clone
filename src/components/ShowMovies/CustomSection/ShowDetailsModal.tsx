...
const ShowDetailsModal = (props: {
  ...
  const [season, setSeason] = useState<number | string>(() => { if (data && data.seasons) { return 0 } return 1 });
  const [multipleSeasons, setMultipleSeasons] = useState<boolean>(() => { if (data && data.seasons && data.seasons.length > 1) { return true } return false });
  const [isMovie, setIsMovie] = useState<boolean>(() => { if (data && data.seasons) { return true } return false });
  ...
  if (!isMovie) {
    ep = useFetchEpisodes({ serieId: props.show.id, seasonNumber: Number(season) })
    // console.log(ep);
  }
            ...
            {dataIsLoading
              ...
              : data && (
                <>
                  ...
                  {ep.data !== null && (
                    <>
                      {ep.data.episodes && ep.data.episodes.map((episode: Episode, index) => (
